...
function parse(gltf, { fileName = 'model', ...options } = {}) {
  ...
  interface GLTFAction extends THREE.AnimationClip {\n
    ...
    const contextType = `type ContextType = Record<string, ForwardRefExoticComponent<
...
${parseExtras(gltf.parser.json.asset && gltf.parser.json.asset.extras)}*/`
  const result = `${options.types ? `\nimport * as THREE from 'three'` : ''}
        import { ${hasAnimations ? 'useRef, ForwardRefExoticComponent,': ''} ${options.ref ? ' forwardRef, Ref ' : ''} ${hasInstances ? ', useMemo, useContext, createContext' : ''} } from 'react'
        ...
        export ${options.exportdefault ? 'default' : ''} ${options.ref ? `${options.exportdefault ? '' : 'const Model ='} forwardRef((` : 'function Model('}props${
          options.types ? ": JSX.IntrinsicElements['group']" : ''
        }${options.ref ? `, ref ${options.types ? ": Ref<THREE.Group>" : ''}) => {` : ') {'}
          ...
          return (
            <group ${options.ref ? 'ref={ref}' : ''} ${hasAnimations ? `ref={group}` : ''} {...props} dispose={null}>
        ${scene}
            </group>
          )
        }${options.ref ? ')' : ''}

${options.exportdata ? `export const UserData = ${JSON.stringify(gltf.scene.children.reduce((acc, val) => { return Object.assign(acc, val.userData); }, gltf.scene.userData))}` : ''}

${options.preload ? `useGLTF.preload('${url}')` : ''}`
